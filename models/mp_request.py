...
class MPProvider():
    ...
    def _set_client(self, wsdl_filename, api):
        wsdl_path = get_resource_path('delivery_mp', 'api', wsdl_filename)
    ...
    def _set_dct_bkg_details(self, carrier, packages):
        ...
        bkg_details.PaymentAccountNumber = carrier.mp_account_number
        if carrier.mp_dutiable:
    ...
    def _set_shipment_details(self, picking):
        ...
        for sequence, package in enumerate(packages):
            ...
            piece.Depth = package.dimension['length']
            piece.Width = package.dimension['width']
            piece.Weight = picking.carrier_id._mp_convert_weight(package.weight, picking.carrier_id.mp_package_weight_unit)
        ...
        if picking.carrier_id.mp_dutiable:
    ...
    def _process_shipment(self, shipment_request):
        ...
        if self.debug_logger:
            self.debug_logger(request_to_send, 'mp_shipment_request')
            self.debug_logger(response.content, 'mp_shipment_response')
    ...
    def _process_rating(self, rating_request):
        ...
        response = self.client._Client__obj.transport.post(self.url, request_to_send, headers=headers)
        if self.debug_logger:
            self.debug_logger(request_to_send, 'mp_rating_request')
            self.debug_logger(response.content, 'mp_rating_response')
        response_element_xml = fromstring(response.content)
        dict_response = {'tracking_number': 0.0,
                         'price': 0.0,
                         'currency': False}
    ...
    def check_required_value(self, carrier, recipient, shipper, order=False, picking=False):
        ...
        if not carrier.mp_SiteID:
            return _("MP Site ID is missing, please modify your delivery method settings.")
        if not carrier.mp_password:
            return _("MP password is missing, please modify your delivery method settings.")
        if not carrier.mp_account_number:
            return _("MP account number is missing, please modify your delivery method settings.")
    ...
    def _set_export_declaration(self, carrier, picking, is_return=False):
        ...
        export_declaration.InvoiceNumber = carrier.env['ir.sequence'].sudo().next_by_code('delivery_mp.commercial_invoice')
        if is_return:
