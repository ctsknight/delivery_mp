...
class Providermp(models.Model):
    ...
    def _rate_shipment_vals(self, order=False, picking=False):
        ...
        rating_request = {}
        srm = MPProvider(self.log_xml, request_type="rate", prod_environment=self.prod_environment)
        ...
        if picking:
            packages = self._get_packages_from_picking(picking, self.mp_default_package_type_id)
        else:
        ...
        self._mp_add_custom_data_to_request(rating_request, 'rate')
        ...
        if qtd_shp:
            for q in qtd_shp:
                ...
                if global_product_code == self.mp_product_code and charge:
    ...
    def mp_send_shipping(self, pickings):
        ...
        for picking in pickings:
            ...
            shipment_request['RegionCode'] = srm._set_region_code(self.mp_region_code)
            shipment_request['RequestedPickupTime'] = srm._set_requested_pickup_time(True)
            shipment_request['Billing'] = srm._set_billing(account_number, "S", self.mp_duty_payment, self.mp_dutiable)
            ...
            total_value, currency_name = self._mp_calculate_value(picking)
            if self.mp_dutiable:
            ...
            shipment_request['LabelImageFormat'] = srm._set_label_image_format(self.mp_label_image_format)
            shipment_request['Label'] = srm._set_label(self.mp_label_template)
            ...
            self._mp_add_custom_data_to_request(shipment_request, 'ship')
            ...
            mp_labels = [('LabelDHL-%s.%s' % (traking_number, self.mp_label_image_format), mp_response.LabelImage[0].OutputImage)]
            mp_cmi = [('DocumentDHL-%s.%s' % (mlabel.DocName, mlabel.DocFormat), mlabel.DocImageVal) for mlabel in mp_response.LabelImage[0].MultiLabels.MultiLabel] if mp_response.LabelImage[0].MultiLabels else None
            ...
            for pick in lognote_pickings:
                ...
                if mp_cmi:
                    pick.message_post(body=_("DHL Documents"), attachments=mp_cmi)
            shipping_data = {
                'exact_price': 0,
    ...
    def mp_get_return_label(self, picking, tracking_number=None, origin_date=None):
        ...
        shipment_request['RegionCode'] = srm._set_region_code(self.mp_region_code)
        shipment_request['RequestedPickupTime'] = srm._set_requested_pickup_time(True)
        shipment_request['Billing'] = srm._set_billing(account_number, "S", "S", self.mp_dutiable)
        ...
        shipment_request['schemaVersion'] = 10.0
        shipment_request['LanguageCode'] = 'en'
        self._mp_add_custom_data_to_request(shipment_request, 'return')
        mp_response = srm._process_shipment(shipment_request)
        traking_number = mp_response.AirwayBillNumber
        logmessage = (_("Shipment created into DHL <br/> <b>Tracking Number : </b>%s") % (traking_number))
